...
uint32_t
algo::hash::crc32b(const uint8_t *octects, int len)
{
...
uint32_t
algo::hash::xcrc32(const uint8_t *octects, int len)
{
	//non standard crc32 implementation
	//refer to the libiberty library
	...
	while (len--) {
		hash = (hash << 8) ^ lookup[((hash >> 24) ^ *octects) & 255];
		octects++;
	}
